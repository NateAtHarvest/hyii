...
class InstallController extends Controller
{
    ...
    public function actionUserManagement()
    {
        ...
        $installMigration = new InstallUserManagement([
        ...
        if ($result) {
            Hyii::_console("Installation of User Management Done! ");
            // $infoId = HyiiHelper::getSystemStateId();
            $connection = Hyii::$app->db;
            $connection->createCommand()->update('{{%info}}',['userManagement' => 'Y'])->execute();

        } else {
            Hyii::_console("Installation of User Management Failed!");
        }
        return ExitCode::OK;
    ...
    {
        $migration = new InstallBlog();
        $result = $migration->safeUp();

        if ($result) {
            Hyii::_console("Installation Done! ");
            $connection = Hyii::$app->db;
            $connection->createCommand()->update('{{%info}}',['blog' => 'Y'])->execute();
        } else {
            Hyii::_console("Installation Failed!");
        }
        return ExitCode::OK;
